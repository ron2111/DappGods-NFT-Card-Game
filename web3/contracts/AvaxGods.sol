...
contract AVAXGods is ERC1155, Ownable, ERC1155Supply {
  ...
  function isPlayer(address addr) public view returns (bool) {
    if(playerInfo[addr] == 0) { // GIVES INFO IF THE PLAYER EXOISTS OR NOT
      return false;
    } else {
      return true;
    }
  ...
  function registerPlayer(string memory _name, string memory _gameTokenName) external {
    ...
    // the numeric values are "player mana and health", flase tells that wwe are not currently in the battle
    playerInfo[msg.sender] = _id; // Creates player info mapping

    createRandomGameToken(_gameTokenName);
    
    emit NewPlayer(msg.sender, _name); // Emits NewPlayer event (we can listen to this on the frontend)
  }
